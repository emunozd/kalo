...
def now_bogota() -> datetime:
    """Retorna datetime actual en zona horaria Bogotá (UTC-5)."""
...
class Usuario(Base):
    ...
    creado_en:          Mapped[datetime]      = mapped_column(DateTime(timezone=True), default=now_bogota)
    actualizado_en:     Mapped[datetime]      = mapped_column(DateTime(timezone=True), default=now_bogota)
...
class RegistroCaloria(Base):
    ...
    registrado_en:Mapped[datetime]      = mapped_column(DateTime(timezone=True), default=now_bogota)  # momento exacto
...
class RegistroEjercicio(Base):
    ...
    registrado_en:  Mapped[datetime] = mapped_column(DateTime(timezone=True), default=now_bogota)  # momento del registro
    descripcion:    Mapped[str]      = mapped_column(Text, nullable=False)
...
class ResumenDiario(Base):
    ...
    primera_entrada_en: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=now_bogota)
    actualizado_en:     Mapped[datetime] = mapped_column(DateTime(timezone=True), default=now_bogota)
